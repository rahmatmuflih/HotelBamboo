<?php
    ...
    if (isset($_SESSION['admin_id'])) {
        echo "<script type='text/javascript'> document.location = 'manage-invoice.php'; </script>";
    }
    if(isset($_POST["login"])){
        $username = $_POST["username"];
        $password = md5($_POST["password"]);
        $sql = "SELECT * FROM resepsionis WHERE username='".$username."' AND password='".$password."'";
        $query = mysqli_query($con,$sql);
        if(mysqli_num_rows($query)>0){
            $row = mysqli_fetch_assoc($query);
            $_SESSION['resepsionis_id'] = $row['resepsionis_id'];
            $_SESSION['nama_resepsionis'] = $row['nama_resepsionis'];
            echo "<script type='text/javascript'> document.location = 'manage-invoice.php'; </script>";
        } else{
        ...
        <div class="row">
          <div class="col-12 col-sm-8 offset-sm-2 col-md-6 offset-md-3 col-lg-6 offset-lg-3 col-xl-4 offset-xl-4">
            <div class="login-brand logo">
              ...
              <h4 style='margin-top:20px;'>Login Receptionist</h4>
            </div>

            <div class="card card-primary">
                <div class="card-header"><h4>Login Receptionist</h4></div>
                ...
                <div class="card-body">
                    ...
                    <div class="text-muted text-center" style="margin-top:10px;">
                        Bukan Receptionist? <a href="../admin/" class="login">Masuk Sebagai Admin</a>
                    </div>
                </div>
            </div>
